...
def train(model, iterator, optimizer, log_vars, online_features_dict, rid_features_dict, parameters):
    ...
    for i, batch in enumerate(iterator):
        ...
        optimizer.zero_grad()
        # flops, params = profile(model, (src_grid_seqs, src_lengths, trg_rids, trg_rates, trg_lengths,
...
def evaluate(model, iterator, rn, rn_dict,
    ...
    with torch.no_grad():  # this line can help speed up evaluation
        for i, batch in enumerate(iterator):
            ...
            output_rates = output_rates.squeeze(2)
            # output_seqs = toseq(rn, output_ids, output_rates, parameters)
            trg_rids = trg_rids.squeeze(2)
            trg_rates = trg_rates.squeeze(2)
            # output_ids = [trg len, batch size, id one hot output dim]
            # output_rates = [trg len, batch size]
        ...
        return epoch_id1_loss / len(iterator), epoch_recall_loss / len(iterator), \
               ...
               epoch_rate_loss / len(iterator), epoch_id_loss / len(iterator), epoch_dis_mae_loss / len(iterator), \
               epoch_dis_rmse_loss / len(iterator), epoch_dis_rn_mae_loss / len(iterator), epoch_dis_rn_rmse_loss / len(
            iterator)
