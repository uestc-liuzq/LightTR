...
def metricandprint(args, algclass, train_loaders, val_loaders, test_loaders, SAVE_PATH, best_acc, best_tacc, a_iter,
                   best_changed):
    # evaluation on training data
    for client_idx in range(args.n_clients):
        train_loss, train_acc = algclass.client_eval(
    ...
    if best_changed:
        ...
        for client_idx in range(args.n_clients):
            _, test_acc = algclass.client_eval(
                client_idx, test_loaders)
            recall, precision, mae, mse = algclass.metrics(client_idx, test_loaders)
            print(
                f' Test site-{client_idx:02d} | Epoch:{best_epoch} | Test Acc: {test_acc:.4f}')
            best_tacc[client_idx] = test_acc
